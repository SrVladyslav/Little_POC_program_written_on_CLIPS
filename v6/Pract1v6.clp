...
(deffunction inicio ()
	...
	(assert (tren P6 1 vagon V1 T1 P6 1 0 vagon V2 T2 P2 0 0 maletas maleta M1 0 maleta M2 0 maleta M3 0 maleta M4 0 nivel 0 entregadas))
...
(defrule final
  ...
  (max-depth ?userDepth)
	?f <- (tren ? ?numVagones $?vt vagon $?v maletas $?vt2 maleta $?vj nivel ?nivel entregadas $?entr)
	      (max-depth ?userDepth)
	(test (= ?numVagones 0))
...
(defrule cargarMaleta
   	(declare (salience 50))
    (max-depth ?userDepth)
   	(tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?pos 1 ?numMaletas $?vt1 maletas $?vt2 maleta ?maleta 0 $?c3 nivel ?nivel entregadas $?entr)
    ...
    (test (< ?nivel ?userDepth))
    (test (> ?numVagones 0))
    (test (eq (member$ ?maleta (create$ a $?entr)) FALSE))
	=>
	(assert (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?pos 1 (+ ?numMaletas 1) $?vt1 maletas $?vt2 maleta ?maleta 1 $?c3 nivel (+ ?nivel 1) entregadas $?entr))
...
(defrule descargarMaleta
   (declare (salience 49))
   (max-depth ?userDepth)
   (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?pos 1 ?numMaletas $?vt1 maletas $?vt2 maleta ?maleta 1 $?c3 nivel ?nivel entregadas $?entr)
   (maleta ?maleta ? ? ?pos)
   (test (> ?numMaletas 0))
   (test (< ?nivel ?userDepth))
    ;Movidas con nombre y tipos de vagon
	=>
	(assert (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?pos 1 (- ?numMaletas 1) $?vt1 maletas $?vt2 maleta ?maleta 0 $?c3 nivel (+ ?nivel 1) entregadas $?entr ?maleta))
...
(defrule desengancharVagon
   (declare (salience 1))
   (max-depth ?userDepth)
   ?f0 <- (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?pos 0 ?numMaletas $?vt1 maletas $?vt2 maleta $?c3 nivel ?nivel entregadas $?entr)
   (test (> ?numVagones 0))
    (test (= ?numMaletas 0))
    (test (< ?nivel ?userDepth))
	=>
